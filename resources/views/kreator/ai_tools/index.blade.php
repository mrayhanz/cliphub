...
<div class="relative z-10 max-w-5xl mx-auto space-y-7 pb-12" x-data="aiClipper()">
    ...
    <div class="flex items-start gap-3 max-w-2xl mx-auto w-full px-4 lg:px-0">
        <div class="flex flex-col items-center text-center gap-2 flex-1">
            <div class="w-[28px] h-[28px] rounded-full bg-violet-500/12 shadow-[0_0_0_1px_rgba(139,92,246,0.25)] text-violet-400 text-[0.7rem] font-black flex items-center justify-center shrink-0">1</div>
            <i data-lucide="link" class="w-4 h-4 text-violet-400"></i>
            <p class="text-[10px] font-semibold text-slate-600">Paste URL</p>
        </div>
        <div class="flex-1 h-px bg-gradient-to-r from-transparent via-violet-500/20 to-transparent self-start mt-[14px]"></div>
        <div class="flex flex-col items-center text-center gap-2 flex-1">
            <div class="w-[28px] h-[28px] rounded-full bg-violet-500/12 shadow-[0_0_0_1px_rgba(139,92,246,0.25)] text-violet-400 text-[0.7rem] font-black flex items-center justify-center shrink-0">2</div>
            <i data-lucide="brain-circuit" class="w-4 h-4 text-fuchsia-400"></i>
            <p class="text-[10px] font-semibold text-slate-600">AI Analisis</p>
        </div>
        <div class="flex-1 h-px bg-gradient-to-r from-transparent via-violet-500/20 to-transparent self-start mt-[14px]"></div>
        <div class="flex flex-col items-center text-center gap-2 flex-1">
            <div class="w-[28px] h-[28px] rounded-full bg-violet-500/12 shadow-[0_0_0_1px_rgba(139,92,246,0.25)] text-violet-400 text-[0.7rem] font-black flex items-center justify-center shrink-0">3</div>
            <i data-lucide="scissors" class="w-4 h-4 text-pink-400"></i>
            <p class="text-[10px] font-semibold text-slate-600">Auto Potong</p>
        </div>
        <div class="flex-1 h-px bg-gradient-to-r from-transparent via-violet-500/20 to-transparent self-start mt-[14px]"></div>
        <div class="flex flex-col items-center text-center gap-2 flex-1">
            <div class="w-[28px] h-[28px] rounded-full bg-violet-500/12 shadow-[0_0_0_1px_rgba(139,92,246,0.25)] text-violet-400 text-[0.7rem] font-black flex items-center justify-center shrink-0">4</div>
    ...
    {{-- INPUT CARD --}}
    <div class="bg-[#0f0f0f] shadow-[0_0_0_1px_rgba(255,255,255,0.05)] border-none rounded-[1.5rem] p-6 relative overflow-hidden before:content-[''] before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-gradient-to-r before:from-transparent before:via-violet-500/50 before:to-fuchsia-500/50">
        ...
        <form @submit.prevent="generate" class="space-y-5">
            <div class="relative">
                ...
                <input x-model="url" type="url" id="video-url" class="w-full bg-[#080808] shadow-[0_0_0_1px_rgba(255,255,255,0.06)] border-none rounded-[0.875rem] py-3.5 pl-11 pr-4 text-[0.875rem] text-slate-200 outline-none transition-shadow duration-200 focus:shadow-[0_0_0_1.5px_rgba(139,92,246,0.55),_0_0_0_4px_rgba(139,92,246,0.08)] placeholder-zinc-700 disabled:opacity-50"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 pt-1">
                ...
                <div class="space-y-2.5">
                    ...
                    <div class="flex gap-2">
                        <button type="button" @click="settings.duration = 'auto'" class="flex-1 py-3 px-2 bg-white/[0.02] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.06)] rounded-xl text-[0.75rem] font-semibold text-zinc-500 cursor-pointer transition-all duration-200 flex items-center justify-center gap-1.5 hover:bg-white/[0.04] hover:text-zinc-300 [&.active]:bg-violet-500/10 [&.active]:text-fuchsia-500 [&.active]:shadow-[inset_0_0_0_1.5px_rgba(217,70,239,0.4)]" :class="settings.duration === 'auto' ? 'active' : ''">Auto</button>
                        <button type="button" @click="settings.duration = '30s'" class="flex-1 py-3 px-2 bg-white/[0.02] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.06)] rounded-xl text-[0.75rem] font-semibold text-zinc-500 cursor-pointer transition-all duration-200 flex items-center justify-center gap-1.5 hover:bg-white/[0.04] hover:text-zinc-300 [&.active]:bg-violet-500/10 [&.active]:text-fuchsia-500 [&.active]:shadow-[inset_0_0_0_1.5px_rgba(217,70,239,0.4)]" :class="settings.duration === '30s' ? 'active' : ''">< 30s</button>
                        <button type="button" @click="settings.duration = '60s'" class="flex-1 py-3 px-2 bg-white/[0.02] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.06)] rounded-xl text-[0.75rem] font-semibold text-zinc-500 cursor-pointer transition-all duration-200 flex items-center justify-center gap-1.5 hover:bg-white/[0.04] hover:text-zinc-300 [&.active]:bg-violet-500/10 [&.active]:text-fuchsia-500 [&.active]:shadow-[inset_0_0_0_1.5px_rgba(217,70,239,0.4)]" :class="settings.duration === '60s' ? 'active' : ''">30s - 60s</button>
                    </div>
                </div>
                ...
                <div class="space-y-2.5">
                    ...
                    <div class="flex items-center justify-between p-2.5 px-3.5 rounded-xl cursor-pointer" 
                        ...
                        <div class="w-9 h-5 bg-zinc-800 rounded-full relative transition-colors duration-300 group" :class="settings.captions ? 'bg-fuchsia-500 active' : ''">
            ...
            <div class="pt-4 flex flex-col md:flex-row items-center justify-between gap-4 shadow-[0_-1px_0_rgba(255,255,255,0.05)] mt-4">
                <div class="text-[11px] text-slate-500 font-medium flex-1 pt-3">
                    <i data-lucide="info" class="w-3.5 h-3.5 inline mb-0.5 text-fuchsia-400"></i>
                ...
                <button type="submit" id="btn-generate" class="w-full md:w-auto shrink-0 px-6 py-3.5 rounded-[0.875rem] text-[0.8rem] font-black tracking-widest uppercase flex items-center justify-center gap-2 cursor-pointer transition-all duration-200 whitespace-nowrap border-none mt-3 md:mt-0"
                        :class="isLoading ? 'bg-[#1a1a1a] text-zinc-700 cursor-not-allowed' : 'bg-gradient-to-br from-violet-600 to-fuchsia-500 text-white shadow-[0_0_24px_rgba(139,92,246,0.35)] hover:shadow-[0_0_32px_rgba(139,92,246,0.5)] hover:-translate-y-[1px]'"
                        :disabled="isLoading">
            ...
            <div x-show="info" x-transition class="p-3 rounded-xl text-xs font-medium flex items-center gap-2 text-violet-300 bg-violet-500/5 shadow-[0_0_0_1px_rgba(139,92,246,0.18)]" style="display:none">
        ...
        <div class="space-y-3">
            ...
            <template x-for="c in pendingClips" :key="c.id">
                <div class="bg-[#0f0f0f] shadow-[0_0_0_1px_rgba(255,255,255,0.05)] border-none rounded-2xl p-4 flex items-center gap-4">
                    <div class="w-9 h-9 rounded-full shrink-0 border-2 border-violet-500/15 border-t-violet-500 animate-[spin_0.9s_linear_infinite]"></div>
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <template x-for="c in doneClips" :key="c.id">
                <div class="bg-[#0f0f0f] shadow-[0_0_0_1px_rgba(255,255,255,0.05)] border-none rounded-[1.25rem] overflow-hidden flex flex-col transition-all duration-200 hover:shadow-[0_0_0_1px_rgba(139,92,246,0.35),_0_8px_30px_rgba(0,0,0,0.4)] hover:-translate-y-[2px]">
                    ...
                    <div class="p-4 flex flex-col gap-3">
                        ...
                        <div class="border-t-0 shadow-[0_-1px_0_rgba(255,255,255,0.04)] pt-3 mt-3 flex items-center justify-between">
                            ...
                            <a :href="c.file_url" :download="c.title + '.mp4'"
                               class="flex items-center gap-1.5 px-3 py-1.5 bg-violet-600 hover:bg-violet-500 text-white text-xs font-bold rounded-lg transition-all shadow-[0_0_12px_rgba(139,92,246,0.3)]">
...
function aiClipper() {
    return {
        ...
        async generate() {
            ...
            try {
                const res = await fetch('{{ route("kreator.ai_tools.process") }}', {
                    method: 'POST',
